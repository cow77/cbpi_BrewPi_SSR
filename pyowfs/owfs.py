...
#    27-Apr-2016 (MPR) Fixed cached
#    26-Jul-2017 (MPR) Added logging, improved cache handling, added shortcut
...
class Dir (object) :
    ...
    def iter_entries (self, cached = True) :
        """Iterates over all entries under this directory node.
        """
        entries = self.capi.get (self.path, cached = cached)
        if entries :
    ...
    def has_key (self, key, cached = True) :
        return key in self.capi.get (self.path, cached = cached).split (",")
    # end def has_key
# end class Dir
...
class Sensor (Dir) :
    ...
    def iter_sensors (self, cached = True) :
        ...
        """
        sensors = self.capi.get (self.path, cached = cached)
        if sensors :
    ...
    def find (self, **kw) :
        ...
        for sensor in self.iter_sensors (cached = cached) :
            if "id" in kw :
                # shortcut when searching for id
                log.debug ("shortcut to %r (path = %r)" % (kw ["id"],sensor.path))
                if sensor.path.endswith (kw ["id"]+"/") :
                    log.debug ("shortcut matched !")
                    res.append (sensor)
                    return res
                else :
                    log.debug ("shortcut not matched")
                    continue
